    ...
    zipped.forEach { (lhs, rhs) in
        ...
        if leftDump != String(dumping: rhs.value) {
            if let notPrimitive = Mirror(reflecting: lhs.value).displayStyle, notPrimitive != .tuple {
                ...
                if !results.isEmpty {
                    closure("child \(lhs.label ?? ""): \((0..<level).reduce("") { acc, _ in acc + "\t" })" + results.joined())
                }
            } else {
...
public func AssertNotDiff<T>(_ expected: T, _ received: T, file: StaticString = #file, line: UInt = #line) {
    var results: [String] = []
    diff(expected, received) { results.append($0) }
    XCTAssertTrue(results.isEmpty, "Found difference for \(results.joined(separator: ", "))", file: file, line: line)
}
